...
# 提取数据框中的score列作为fc向量
# Extract the 'score' column as fc vector
fc <- dat$score

# 将数据框的行名作为fc向量的名称
# Assign row names as names of fc vector
names(fc) <- rownames(dat)

### -log10P列 ###
### -log10P Column ###

# 提取数据框中的-log10P列作为p向量（注意反引号用于特殊列名）
# Extract the '-log10P' column as p vector (backticks for special column names)
p <- dat$`-log10P`

# 将数据框的行名作为p向量的名称
# Assign row names as names of p vector
...
# 为每个pathway组别分配颜色
# Assign colors for each pathway group

# 自定义颜色向量（16进制颜色代码）
# Define custom color vector (hex color codes)
mycol <- c("#B2DF8A","#FB9A99","#33A02C","#E31A1C","#B15928","#6A3D9A",
           "#CAB2D6","#A6CEE3","#1F78B4","#FDBF6F","#999999","#FF7F00")

# 获取数据中所有唯一的group类别
# Get all unique group categories in the data
cols.names <- unique(dat$group)

# 为每个group类别分配颜色（按顺序取mycol中的颜色）
# Assign colors to each group (taking colors from mycol in order)
cols.code <- mycol[1:length(cols.names)]

# 将group类别名称作为颜色向量的名称
# Name the color vector with group categories
names(cols.code) <- cols.names

# 为每条记录创建带透明度的颜色（"BB"表示约75%透明度）
# Create colors with transparency for each record ("BB" means ~75% opacity)
col <- paste(cols.code[as.character(dat$group)],"BB", sep="")

# 标记需要高亮的特定pathway（PathwayA, C, H, E）
# Identify specific pathways to highlight (PathwayA, C, H, E)
i <- dat$group %in% c("PathwayA","PathwayC","PathwayH","PathwayE")

### size列 ###
### size Column ###

# 提取数据框中的size列作为sizes向量
# Extract the 'size' column as sizes vector
sizes <- dat$size

# 将数据框的行名作为sizes向量的名称
# Assign row names as names of sizes vector
...
# 提取数据框中的pval列作为pp向量
# Extract the 'pval' column as pp vector
pp <- dat$pval

# 将数据框的行名作为pp向量的名称
# Assign row names as names of pp vector
