...
To cite the method in publications use:

Yang, C., Zhang, S., Cheng, Z. et al. Multi-region sequencing with spatial information enables accurate heterogeneity estimation and risk stratification in liver cancer. Genome Med 14, 142 (2022). https://doi.org/10.1186/s13073-022-01143-6
...
FigureYa296VIPER跟这个不一样。这篇里面用的是新版的weighted VIPER，和之前的算法不一样，而且这个图重点是IHS分数的计算，不仅是蛋白活性的计算。

I'd like to draw Figure 5D from this article. The author developed a method to calculate gene heterogeneity, then used VIPER to infer checkpoint protein activity before calculating the heterogeneity of checkpoint proteins. I think this method could also be applied to calculate heterogeneity for other types of proteins - it seems quite practical.

FigureYa296VIPER is different from this one. Here, they used the new weighted VIPER, which has a different algorithm from the previous version. Moreover, the focus of this figure is on the calculation of IHS scores, not just protein activity inference.
...
library(viper) # 计算蛋白活性 To calculate protein activity
library(nlme) # 计算IHS To calculate IHS
library(spatstat.explore) # 计算IHS To calculate IHS
library(ggplot2) # 画图 To draw plots
library(ggrepel) # 避免标签重叠 To avoid label overlap 

# 显示英文报错信息 To show English error messages
Sys.setenv(LANGUAGE = "en") 

# 禁止chr转成factor To prevent character to factor conversion
options(stringsAsFactors = FALSE) 
```

自定义函数以计算IHS To define a custom function to calculate IHS
...
Since 200 bootstrap iterations are required, the process will take approximately 2 days to complete. Please wait patiently.
...
这里会生成`out_unPruned.rds`文件，用于下一步计算。

An `out_unPruned.rds` file will be generated here, which is used for the next step of calculation.
...
The higher the integrated heterogeneity score (IHS), the greater the intratumor heterogeneity within patients.
