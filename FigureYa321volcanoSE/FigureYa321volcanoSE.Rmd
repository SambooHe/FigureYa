...
To use the PDUI data cite:

Li, L. TC3A_PDUI. Preprint at https://doi.org/10.7303/SYN24982198 (2021).
...
From <https://www.frontiersin.org/articles/10.3389/fimmu.2022.1031184/full>
...
TC3A (The Cancer 3′ UTR Atlas) 专门分析了TCGA 33种癌症类型的3′UTR长度，系统注释了可变多聚腺苷化位点APA (Alternative Polyadenylation, 可变多聚腺苷化)。
...
TC3A (The Cancer 3′ UTR Atlas) specifically analyzes the 3′UTR length of 33 cancer types in TCGA and systematically annotates alternative polyadenylation (APA) sites.
...
- One group: FigureYa59volcano ，FigureYa75baseVolcano  
...
library(limma) # 线性模型拟合 Linear model fitting 
library(ggplot2) # 绘图 Plotting
library(ggrepel) # 标签标注 Labeling
library(TCGAbiolinks) # TCGA数据下载 TCGA data download
...
## 输入文件
## Input Files
TCGA_PAAD_Combined_PDUIs.txt，从synapse网站中下载的TC3A_PDUI（Percentage of Distal polyA site Usage Index）的计算结果。下载地址：<https://www.synapse.org/Synapse:syn24982198/files/>（科学上网后注册下载），里面有TCGA其他癌症的PDUI数据，根据自己的研究兴趣灵活运用。

TCGA_PAAD.clin.txt，样本分组信息group。原文未提供分型信息，使用xena<https://xenabrowser.net/datapages/>下载的样本信息（TCGA Pancreatic Cancer (PAAD) (17 datasets) —— phenotype —— Phenotypes (n=196) TCGA Hub），点击链接下载：<https://tcga-xena-hub.s3.us-east-1.amazonaws.com/download/TCGA.PAAD.sampleMap%2FPAAD_clinicalMatrix>
...
# 设置颜色分组
# Color grouping
plot.data$status <- "NoSig"
plot.data$status[plot.data$logFC < (-0.05) & plot.data$logPadj > 1] <- "Down"
plot.data$status[plot.data$logFC > 0.05 & plot.data$logPadj > 1] <- "Up"
plot.data$status <- factor(plot.data$status, levels = c("Up", "Down", "NoSig"))
...
# 绘制火山图
# Draw volcano plot
ggplot(data = plot.data, aes(x = logFC, y = logPadj, color = status)) + 
  geom_point(alpha = 0.9, size = 2.2) +
  scale_color_manual(values = palette_volcano, drop = FALSE) +
  scale_size_continuous(range=c(1, 8)) +
  geom_label_repel(
    data = subset(plot.data, !is.na(label)),
...
# 保存图片
# Save plot
ggsave("volcano.pdf", width = 6, height = 4)
```
