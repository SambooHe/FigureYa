...
library("ScBulkCCCI")         # 单细胞与bulk数据相关性细胞互作分析｜Correlative cell-cell interaction analysis for scRNA-seq and bulk RNA-seq
library("pheatmap")           # 热图可视化｜Heatmap visualization
library("RColorBrewer")       # 调色板与配色方案｜Color palettes for plots
library("ggplot2")            # 数据可视化与绘图｜Data visualization and plotting
library("ggthemes")           # ggplot2主题美化｜Themes for ggplot2
library("plyr")               # 数据整理与分组操作｜Data manipulation and grouping
library("dplyr")              # 高效数据处理与管道操作｜Efficient data manipulation and piping
library("stringr")            # 字符串处理与正则表达式｜String manipulation and regular expressions
library("tibble")             # 增强型数据框结构｜Enhanced dataframe structure
library("tidyr")              # 数据整理与宽长格式转换｜Data tidying and reshaping
# 显示英文报错信息
...
Preprocessed data (COAD and READ) downloaded from [UCSC Xena](xenabrowser.net). The preprocessing code is available in the ScBulkCCCI package under example/ScBulkCCCI_DataPreprocess.R. The processed data is bundled in the R package and can be directly loaded.
...
Refer to the example section "Identification of signature genes". It describes the Seurat-based selection criteria. For single-cell data preprocessing and standard Seurat workflow, refer to FigureYa177RNAvelocity.

For highly heterogeneous samples, refer to FigureYa223scNMF. Instead of Seurat, it uses NMF and clustering to identify marker genes.

Here we directly provide the curated Table S4 of the paper[Table S4 of paper](#https://ars.els-cdn.com/content/image/1-s2.0-S009286742030341X-mmc4.xlsx), which has been packaged into the R package and can be loaded directly.

CCgene_normal.rds: Signature genes for immune cell populations enriched in normal tissues; Column 1: gene names, Column 2: cell population names

CCgene_tumor.rds: Signature genes for immune cell populations enriched in tumor tissues; Column 1: gene names, Column 2: cell population names
...
> hM13_TAM-SPP1的富集分析结果在ScBulkCCCI中与文献报道一致。
...
Below we present the Enrichment analysis results visualized using two network tools: igraph (static) and networkD3 (interactive).
...
Open the 'network.html' file and you can view the dynamic network.
