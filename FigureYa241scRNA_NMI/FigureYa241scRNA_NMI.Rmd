...
例文的Figure 2D和G，利用cNMF提取expression programs，并通过整合其它已发表的数据，利用Gene set analysis来推断expression programs的生物学意义。做法可参考FigureYa226scRNA_cNMF
In Figures 2D and 2G of the example paper, expression programs were extracted using cNMF. Gene set analysis was then used to infer the biological significance of these expression programs by integrating with other published data. For instructions, see FigureYa226scRNA_cNMF 
...
library(ggsignif)      # 用于添加显著性标记的包 # For adding significance annotations
library(doParallel)    # 用于并行计算的包 # For parallel computing
library(entropy)       # 用于计算信息熵和互信息的包 # For calculating entropy and mutual information
library(Seurat)        # 用于单细胞数据分析的包 # For single-cell data analysis
library(rhdf5)         # 用于读取和写入HDF5文件的包 # For reading and writing HDF5 files
library(tidyverse)     # 用于数据操作和可视化的包 # For data manipulation and visualization
library(infotheo)      # 用于信息论分析的包 # For information-theoretic analysis
...
100次抽样计算
100 sampling calculations

1次抽样计算(100 cells)大约需要30s，100次抽样计算大约需要50min。这个计算时间成本太高 <br>
两种解决方案：One sampling calculation (100 cells) takes about 30 seconds, and 100 sampling calculations take about 50 minutes. This computational cost is too high. <br>
1) 对cell pairs也进行抽样，降低计算时间 1) Sample cell pairs to reduce computational time <br>
令sample.nmi()中的sample.on.cell.pairs=TRUE Set sample.on.cell.pairs=TRUE in sample.nmi(). <br>
2) 并行计算 Parallel computation. <br>
...
并行计算需要`doParallel`包
Parallel computing requires `doParallel` package

```{r}
## ==== do parallel start ====
## apply for 20 workers
cl <- makeCluster(20) # 设定CPU数量 # Set the number of CPUs
