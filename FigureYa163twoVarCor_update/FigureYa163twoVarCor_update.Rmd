...
- 用点的大小展示横纵坐标的差异，类似于FigureYa59volcano用点的大小体现差异倍数和pvalue；
...
- The size of the dot indicates the difference between the horizontal and vertical axes, similar to FigureYa59volcano, which uses the size of the dot to indicate the fold difference and p-value;
...
- Here, each group is distinguished by color. If there are too many groups to distinguish, refer to FigureYa125 Fishertest, FigureYa76 corrgram, or FigureYa97 correlationV3. For even more, use FigureYa73 batchCorrelation for batch correlation analysis and plotting.
...
Compare the correlations between different groups to see if they are statistically significant.

直接在屏幕上打印两次相关性的差异显著性

Print the significance of the difference between the two correlations directly to the screen.
...
First, calculate the additional variables needed for plotting.
...
plot(-1, -1,
     ...
     xaxt = "n", yaxt = "n")
invisible(dev.off())
# 最终图可在Illustrator等矢量图编辑器里稍加修饰
# Final plot can be slightly modified in Illustrator and other vector graphics editors
```
