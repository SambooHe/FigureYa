...
## 第一部分：用例文提供的数据画图
## Part 1: Draw a graph using the data provided in the example text

### 输入文件的准备
### Preparation of input file

如果你的数据已经整理成easy_input.csv的格式，就可以跳过这步，直接进入“开始画图”。
If your data has been organized into the format of easy_input.csv, you can skip this step and go directly to "Start drawing".

输入文件1-s2.0-S1535610818301107-mmc3.xlsx，下载自：<https://www.sciencedirect.com/science/article/pii/S1535610818301107#mmc3>
Input file 1-s2.0-S1535610818301107-mmc3.xlsx, downloaded from: <https://www.sciencedirect.com/science/article/pii/S1535610818301107#mmc3>

第2列是Gene Name，第4列以后是每种癌症类型里的比例。第1、3列非必须。
The second column is the Gene Name, and the fourth and subsequent columns are the proportions of each cancer type. The first and third columns are optional.

把这两个文件整理成画图需要的格式ggData。
Arrange these two files into the format ggData required for drawing.
...
easy_input.csv, the first column of gene name corresponds to a row in the graph, the second column of disease name corresponds to 1 column in the graph, the third column of value corresponds to the proportion of pie, and the fourth column of type corresponds to the grouping in the graph.
...
- cancergenes_list.txt, a list of cancer genes of interest. Download from: <http://ncg.kcl.ac.uk/cancer_genes.php#known>

- TCGA数据用CGDS-R下载，CGDS-R的用法可参考这篇：<https://mp.weixin.qq.com/s/pqck8jMk-WIZ6EgkBIWQHA>，以methylation和mRNA expression为例
- TCGA data are downloaded using CGDS-R. For the usage of CGDS-R, please refer to this article: <https://mp.weixin.qq.com/s/pqck8jMk-WIZ6EgkBIWQHA>, taking methylation and mRNA expression as examples
