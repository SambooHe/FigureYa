...
想实现图5A B, 首先对TCGA数据集进行聚类分析，之后将GDSC细胞系按照TCGA聚类结果进行分组，分组后比较分组的药物敏感性，用AUC定量分析。
...
# TCGA数据分型 - 无监督共识聚类 TCGA Data Typing - Unsupervised Consensus Clustering

使用原文定义的36个essential genes（筛选基因的方法可参考FigureYa240CRISPR），为TCGA的patient进行无监督共识聚类（consensus clustering），这里分成3个亚型，然后画个热图看效果。 Using the 36 essential genes defined in the original paper (see FigureYa240CRISPR for the selection method), perform unsupervised consensus clustering on TCGA patients into 3 subtypes, then plot a heatmap to visualize.
...
筛选出同时具有药敏数据和表达数据的LUAD细胞系 Select LUAD cell lines that have both drug sensitivity and expression data

```{r}
## 读取细胞系注释文件 | read cell line annotations
ccle.anno <- read.delim("Cell_lines_annotations_20181226.txt",sep = "\t",row.names = 1,check.names = F,stringsAsFactors = F,header = T)
ccle.anno <- ccle.anno[which(ccle.anno$tcga_code == "LUAD" & ccle.anno$Hist_Subtype1 == "adenocarcinoma"),] # 选取LUAD并且病理亚型为腺癌的样本（和原文的数量有些出入，也许是我对病理亚型理解不到位） | select LUAD and adenocarcinoma subtype (counts may differ from paper)

## 读取细胞系药敏并匹配DepmapID | read drug sensitivity and match DepMap IDs
gdsc <- fread("sanger-dose-response.csv",check.names = F,stringsAsFactors = F,header = T,data.table = F)
gdsc <- gdsc[,c("ARXSPAN_ID","DRUG_NAME","AUC_PUBLISHED")] # 提取细胞系的DepmapID（细胞系编号），药物名以及对应AUC值 | extract DepMap ID, drug name, and AUC
gdsc.auc <- reshape(gdsc, idvar = "ARXSPAN_ID", timevar = "DRUG_NAME", direction = "wide") # 将long型数据转换为wide型数据 | long-to-wide
...
构建nearest centroid classifier，然后用classifier预测GDSC细胞系数据，让细胞系按照TCGA分型结果分成对应的三组。 Build a nearest centroid classifier from TCGA and use it to predict GDSC cell lines, assigning them to the three TCGA-derived groups.
...
原文采用中心化来构建预测模型，但是我这里的结果不是很好，所以换用z-score标准化的结果。 The original paper used centering to build the classifier; results here were suboptimal, so z-score standardization was used instead.
...
# 药物敏感性的Kruskal检验 Kruskal-Wallis test for drug sensitivity

计算GDSC细胞系各亚型对每种药物的敏感性，输出统计结果并画图。 Compute sensitivity per subtype for each drug, output stats, and plot.

由GDSC细胞系各亚型的敏感性结果可以反推出TCGA各亚型对药物对敏感性。 Sensitivity patterns in GDSC-assigned subtypes can be extrapolated back to TCGA subtypes.
