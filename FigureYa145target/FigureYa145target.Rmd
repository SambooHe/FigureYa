...
According to the number of glycosites, they are divided into 5 categories, with 4 and 5 in one group and >5 in another group, corresponding to the legend on the right.
...
seg.clu is mainly used to draw the last little bit of yellow of Glycan
...
The output PDF file is a vector image, which can be opened with vector image editors such as PS or AI to edit graphics and text.

其实本来文字和数字都是可以在R中加的，但是还需要计算位置，莫不如在PS里修改。这里有一个PS的中间文件可以参考`target.psd`和`target.tif`。
Actually, text and numbers can be added in R, but the position needs to be calculated, so it is better to modify them in PS. Here is a PS intermediate file for reference `target.psd` and `target.tif`.

在用到自己数据的时候，最好先把原数据跑一遍，理解每个数字的含义，和整体的思路，再去举一反三。这里涉及到一些数学的三角函数计算，和角度的计算，还是要理解每个数字的含义的。
When using your own data, it is best to run the original data first, understand the meaning of each number, and the overall idea, and then draw inferences from one example. This involves some mathematical trigonometric function calculations and angle calculations, and you still need to understand the meaning of each number.
