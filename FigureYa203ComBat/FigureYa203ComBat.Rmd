...
We would like to process the data as the methods described in this article, specifically the merging and normalization of TCGA RNA-seq data and microarray data from different platforms, hoping to achieve a relatively unified processing approach.
...
# 1. TPM
# 1. TPM
tpmMatrix <- assay(expdata, "tpm_unstrand")
tumorSamples <- colnames(tpmMatrix)[substr(colnames(tpmMatrix), 14, 15) == "01"]
tpmMatrix <- tpmMatrix[, tumorSamples]

tpms <- as.data.frame(round(tpmMatrix, 2))

# 2. 基因注释（假设已有overlapTable27.txt文件）
# 2. Gene annotation (assuming overlapTable27.txt exists)
...
（还可以用`FigureYa59Plus_GEO2DEG`下载GEO芯片数据并预处理，获得表达矩阵easy_input_expr.csv文件）
...
(Alternatively, you can use `FigureYa59Plus_GEO2DEG` to download and preprocess GEO microarray data, obtaining the expression matrix file easy_input_expr.csv)
