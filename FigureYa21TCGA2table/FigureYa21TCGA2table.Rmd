...
library(TCGAbiolinks)  # 用于从TCGA数据库下载和准备数据的包 # For downloading and preparing data from the TCGA database
library(dplyr)         # 用于数据操作和处理的包 # For data manipulation and processing
library(kableExtra)    # 用于生成增强表格输出的包 # For generating enhanced table outputs
```
