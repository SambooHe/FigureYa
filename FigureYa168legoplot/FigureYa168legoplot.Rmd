...
The Lego chart is based on the source code of the barplot3d package, with some modifications. The link to this R package is <https://cran.r-project.org/web/packages/barplot3d/vignettes/barplot3d.html>

very_easy_input.csv，包含两列，第一列mut_type对应图中的6种颜色，第二列mut_context对应每种颜色中的16个柱子，柱子的高度代表数量。套用自己的数据即可。
very_easy_input.csv contains two columns: the first column, mut_type, corresponds to the six colors in the chart, and the second column, mut_context, corresponds to the 16 bars in each color. The height of the bar represents the number of bars. Simply apply your own data.
...
Sometimes, we want to display multiple 3D plots in the same article at the same angle. To do this, save the angle and use it each time you draw.
...
载入之前调好的角度画图
Load the previously adjusted angle to draw the graph
...
输出的PDF文件可以使用AI打开，拼接
The output PDF file can be opened and spliced using AI.
