...
Then import the FGA data from cBioPortal to verify the results of the algorithm.
...
先贴上计算公式便于理解：
First, attach the calculation formula for easier understanding:

![](formula.png)

因此，Segment中的value值即是log2R。如果是自己的测序数据则要注意是否是log2R，还是copy number（见文末的注意事项）。
Therefore, the value in the Segment is log2R. If it is your own sequencing data, pay attention to whether it is log2R or copy number (see the precautions at the end of the text).
...
Conclusion: The majority of the data are consistent. The differences in individual samples are due to the different cnv files used (see the notes at the end of the text).
...
Calculate the mean, se and p values and draw graphs. The test method used here is AONVA, and TukeyHSD is used for inter-group comparison (the results only take the significance of the reference group and other groups for easy display).
...
The P-value and se are not marked in Figure 1 of the literature, so a custom advanced version of the FGA diagram is drawn.
...
The calculation of FGA relies on cnv data, namely the segmentation file generated by CNV calling. So different CNV calling processes and different parameter Settings will generate different cnv data (specifically, you can refer to Start, End, Num_Probes, Segment_Mean in the egmentation file). This is also the reason why the FGA of some patients in the above calculation results differs from that of cbioportal.
...
Besides, If you want to calculate the cnv data from TCGA by yourself, please note that only TCGA on xena (not GDC) and **SNP_N_GenomeWideSNP_6_D01_680066.nocnv_grch38.seg.v2.txt** on the GDC official website can be used with cbiopor Consistency on tal (differences in the C11b version).
...
Download segmentation from the GDC official website The log2R Segment_Mean and formula in the file (website: < https://docs.gdc.cancer.gov/Data/Bioinformatics_Pipelines/CNV_Pipeline/ >).

如果自己的测序数据则要注意，如果是copy number还需要按照上述公式进行转换。
If it is your own sequencing data, you should pay attention. If it is a copy number, it also needs to be converted according to the above formula.
