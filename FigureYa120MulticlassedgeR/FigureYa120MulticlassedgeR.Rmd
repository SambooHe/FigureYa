...
Implement multi group difference analysis using EdgeR. Can be used to replace the paired differential expression process (subtype number>=3) in FigureYa109SubtypeGSEA and seamlessly integrate with it.
...
**注意：**本次不涉及批次效应消除，若样本间含有批次效应，请阅读edgeR的document，在设计矩阵中纳入batch effect。<https://bioconductor.org/packages/release/bioc/vignettes/edgeR/inst/doc/edgeRUsersGuide.pdf>，有个完整的例子：4.2 RNA-Seq of pathogen inoculated arabidopsis with batch effects
...
**Note: * * This time does not involve batch effect elimination. If there are batch effects between samples, please read the edgeR document and include batch effects in the design matrix< https://bioconductor.org/packages/release/bioc/vignettes/edgeR/inst/doc/edgeRUsersGuide.pdf >Here is a complete example: 4.2 RNA Seq of pathogenic uncoordinated arabidopsis with batch effects

For the case where the read count expression matrix is used as input, FigureYa118MulticlassDESeq2 can also be used. If your data is FPKM/RPKM or chip data, please refer to FigureYa119 Multiclasslimma.
...
Customize the function to compare the differences between each group and other samples separately. If there are more groups, simply add them according to the pattern.
...
```{r}
### 创建配对比较的列表信息 ###
### Create pairwise comparison list information ###

# 提取亚型信息并命名 [Extract subtype information and assign names]
group <- subt$TCGA_Subtype  # 提取亚型列 [Extract subtype column]
names(group) <- rownames(subt)  # 设置样本名为亚型向量的名称 [Set sample names as names of subtype vector]

# 创建比较列表 [Create comparison list]
# 参数：group - 命名的亚型向量 [Parameter: group - Named subtype vector]
complist <- createList(group=group)  # 调用之前定义的createList函数 [Call previously defined createList function]

### 执行配对DESeq2差异表达 ###
### Perform pairwise DESeq2 differential expression analysis ###

# 计算基因表达的标准差 [Calculate standard deviation of gene expression]
var <- apply(expr,1,sd)  # 对每一行(基因)计算标准差 [Calculate SD for each row (gene)]

# 差异表达分析过程比较慢请耐心等待，这里为了加速分析过程，只选取方差top25%的基因
# Differential expression analysis is time-consuming. To speed up, select top 25% genes by variance
index <- var > quantile(var)[4]  # 筛选方差大于第75百分位数的基因 [Select genes with variance > 75th percentile]

# 执行配对edgeR差异表达分析 [Perform pairwise edgeR differential expression analysis]
# 参数说明：
# res.path = "." - 结果输出路径为当前目录 [Results output path is current directory]
# countsTable - 输入的表达矩阵，使用筛选后的基因和匹配的样本 [Input expression matrix with filtered genes and matched samples]
# prefix = "SKCM" - 结果文件名前缀 [Result file name prefix]
# complist - 比较列表 [Comparison list]
# overwt = F - 不覆盖已存在的结果文件 [Do not overwrite existing result files]
twoclassedgeR(res.path = ".", #所有配对差异表达结果都会输出在res.path路径下
              # All pairwise DE results will be output to res.path directory
              countsTable = expr[index,intersect(colnames(expr),rownames(subt))],
              # 如果不取方差top25%，而是计算全部基因，就删掉上面那行，改为运行下面这行：
              # If you want to analyze all genes instead of top 25% by variance, comment the line above and uncomment the line below:
              #countsTable = expr,
              prefix = "SKCM", #文件名以SKCM开头 [Result files start with SKCM]
              complist = complist,
              overwt = F)
```
