...
哪个容易讲故事，最后就选哪个方法、哪个数据库。然后继续做深入研究。
...
Which is easy to tell a story, in the end, choose which method and which database. Then go ahead and do in-depth research.
...
```{r}
# 查看msigdbr所支持的物种 / See which species are supported by MSIGDBR
# 这里没有我要的物种怎么办？看方法三 / What should I do if I don't have the species I want here? Look at method three
...
gmt <- msigdbr(species = target_species) 

find_column <- function(data, pattern) {
  col <- grep(pattern, colnames(data), ignore.case = TRUE, value = TRUE)
  if (length(col) == 0) {
    stop("找不到匹配 '", pattern, "' 的列")
  }
  return(col[1])
}

gs_name_col <- find_column(gmt, "gs.*name")
entrez_col <- find_column(gmt, "entrez")
gs_cat_col <- find_column(gmt, "gs.*cat")

gmt2 <- gmt[, c(gs_name_col, entrez_col)]
colnames(gmt2) <- c("gs_name", "entrez_gene")

# 按基因集类别分组
gmts <- split(gmt2, gmt[[gs_cat_col]])
cat("创建了", length(gmts), "个基因集类别 / Created", length(gmts), "gene set categories\n")
...
easy_input_limma.csv, the results of differential expression analysis. is the output file of FigureYa59Plus_GEO2DEG, which can be seamlessly connected.
...
查看clusterProfiler的最新用法，请移步Guangchuang Yu的在线电子书：<https://yulab-smu.top/clusterProfiler-book/>
To see the latest usage of clusterProfiler, please visit Guangchuang Yu's online eBook: <https://yulab-smu.top/clusterProfiler-book/>
...
easy_input_limma.csv, as above, differential expression analysis results. is the output file of FigureYa59Plus_GEO2DEG, which can be seamlessly connected
