...
- 1. Oceania.csv contains overall information for Oceania, including id for each country's specific region or city (REGISTRY), start and end years of statistics (YEAR1 and YEAR2).
- 2. Original data Oceania_Pos.csv contains specific population data for each city's each age group each year.
- 3. Original data Oceania_Cases.csv contains specific case data for each city's each age group each year.

Notes:

- 1. The population and case data (CI5plus dataset) in this code comes from the website <http://ci5.iarc.fr> provided in references of the paper "Global Patterns and Trends in Gastric Cancer Incidence Rates (1988-2012) and Predictions to 2030".
- 2. Since we couldn't find the WHO world population data for standardization used in the paper, we used data from <https://seer.cancer.gov/stdpopulations/world.who.html>, which may differ from the paper.
- 3. Here we use Oceania as an example, other continents follow similar analysis process.
- 4. The entire code takes about 10 minutes to run.
...
```{r}
### 估计每个国家的年龄标准化疾病发生率
### Estimate age-standardized disease incidence rate for each country

# who_pop: 世界卫生组织标准人口分布，基于2000-2025年间各年龄组的世界平均人口
# who_pop: WHO standard population distribution based on world average population for each age group during 2000-2025
who_pop <- c(8.86, 8.69, 8.60, 8.47, 8.22, 7.93, 
             7.61, 7.15, 6.59, 6.04, 5.37, 4.55, 
             3.72, 2.96, 2.21, 1.52, 0.91, 0.635)/100

# 估计每个国家每年各年龄组的疾病率
# Estimate disease rate for each country's each age group each year
p.est <- cases[,3:20] / pops[,3:20] 

# 使用“who_pop”数据，估计每个国家年龄标准化疾病率
# Use "who_pop" data to estimate age-standardized disease rate for each country
asr.est <- (as.matrix(p.est) %*% who_pop) * 100000 
...
### 对每个国家进行BAPC分析预测
### Perform BAPC analysis and prediction for each country
asr.pred <- NULL
for (id in c("Australia", "New_Zealand")) {
  set.seed(10)
  
  # 提取当前国家病例数据
  # Extract current country's case data
  cases_c <- cases[cases[,1] == id, ] 
  
  # 提取当前国家人口数据
  # Extract current country's population data
  pops_c <- pops[pops[,1] == id, ] 
  
  # 准备用于BAPC分析的病例数据，数据类型为矩阵
  # Prepare case data for BAPC analysis (matrix format)
  cases_bamp <- matrix(unlist(cases_c[, 3:dim(cases)[2]]), 
                       dim(cases_c)[1], dim(cases_c)[2]-2) 
  
  # 准备用于BAPC分析的人口数据，数据类型为矩阵
  # Prepare population data for BAPC analysis (matrix format)
  pops_bamp <- matrix(unlist(pops_c[, 3:dim(cases)[2]]), 
                      dim(pops_c)[1], dim(pops_c)[2]-2) 
  
  # 使用函数bamp()估计BAPC模型
  # Use bamp() function to estimate BAPC model
  ...
  per <- 2030-cases_c[dim(cases_c)[1], 2] 
  
  # 预测当前国家各年龄组的疾病率
  # Predict disease rate for each age group of current country
  ...
  # 存储当前国家年龄标准化疾病率预测值
  # Store predicted age-standardized disease rate for current country
  asr.pred <- rbind(asr.pred, data.frame(country = rep(id, per), 
