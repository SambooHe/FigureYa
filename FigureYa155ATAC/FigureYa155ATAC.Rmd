...
peak注释到基因层面，主要用Guangchuang Yu的ChIPseeker。
Peaks are annotated at the gene level, primarily using Guangchuang Yu's ChIPseeker.
...
rawCount.txt, generated earlier. Each line contains one gene. The first six columns contain gene location information, and the following columns each contain the read count for a sample. This can be compared to RNA-seq read count analysis for differential analysis.

先筛选显著差异基因，然后画图。
First, screen for significantly differentially expressed genes, then plot the graph.
...
There are a few remaining loci here. The original paper used pvalue<=0.05 for comparisons, rather than corrected p-values. Following the original paper would yield more genes. However, the six Abca1 loci mentioned in the original paper are still missing.

一种猜测就是原文做了4次生物学重复，而作者可能在其中选择某几个重复去做，而不是全部，然后通过不断的尝试，选在最能证明观点的数据进行可视化。
One hypothesis is that the original paper used four biological replicates, and the authors may have selected a few of these replicates, rather than all, and then, through repeated trial and error, selected the data that best supported their findings for visualization.
