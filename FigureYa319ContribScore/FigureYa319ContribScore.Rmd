...
library(Seurat)        # 用于单细胞数据分析 # For single-cell data analysis
library(magrittr)      # 提供管道操作符 %>% # Provides the pipe operator %>%
library(GEOquery)      # 用于从GEO数据库下载数据 # For downloading data from the GEO database
library(SingleR)       # 用于单细胞数据的自动注释 # For automatic annotation of single-cell data
library(plyr)          # 用于数据操作和汇总 # For data manipulation and summarization
library(openxlsx)      # 用于读取和写入Excel文件 # For reading and writing Excel files
library(GSVA)          # 用于基因集变异分析 # For gene set variation analysis
