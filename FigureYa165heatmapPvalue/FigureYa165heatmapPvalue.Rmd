...
If your data has already been organized into easy_input* file format, skip directly to "Differential Expression Analysis."
...
```{r}
## 读取感兴趣的基因表达矩阵
## Read expression matrix of the gene of interest
mygene_data <- read.csv("easy_input_expr.csv", row.names = 1)
# 或者读取全部基因的表达矩阵，后面画Top 20个基因
# Or read the expression matrix of all genes and plot the top 20 genes later
...
# View the names and sample numbers of each group. This is used when plotting.
table(Subtype$Subtype)

## 取表达矩阵和分组信息中共有的sample
## Extract samples shared by the expression matrix and grouping information
...
## 用前面的自定义函数计算组间统计差异
## Use the custom function above to calculate statistical differences between groups
comprTab <- cross_subtype_compr(expr = mygene_data, # 或log2(mygene_data + 1)，如果用参数检验，请注意对数转化；若非参均可  If using a parametric test, please note the logarithmic transformation; non-parametric tests are also acceptable.
                                subt = Subtype,
...
The output PDF file is a vector image. You can open it in a vector graphics tool like Illustrator and add the names of each subtype above the annotation bar, following the example text.
