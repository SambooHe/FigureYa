...
library(SingleCellExperiment) # 单细胞实验数据结构｜Single-cell experiment data structure
library(reshape2)             # 数据重塑与转换｜Data reshaping and transformation
library(Seurat)               # 单细胞分析平台｜Single-cell analysis platform
library(igraph)               # 网络分析与图结构处理｜Network analysis and graph handling
library(ggraph)               # 网络图可视化扩展｜Network graph visualization extension
library(ggrepel)              # ggplot标签自动避让｜Automatic label repelling for ggplot
library(RColorBrewer)         # 专业调色板｜Professional color palettes
library(grid)                 # 网格图形系统｜Grid graphics system
