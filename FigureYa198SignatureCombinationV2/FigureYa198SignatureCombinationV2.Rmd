...
我想实现下文章Figure 1c的计算方法，FigureYa182RFSurv挑出了importance比较大的基因，后续常规用muticox等方法再缩减基因。这篇文章写的是用1023种组合按KM的 logRank pvalue和基因数目挑选模型，不一样的风格。
...
专门用于多基因签名的智能组合优化，核心方法是通过排列组合策略筛选最优基因组合。

缺点是：如果基因数量超过15-20个，计算量会变得不现实。

适合在已经通过其他方法初步筛选后的小规模基因集上使用。
...
The disadvantage is that if the number of genes exceeds 15-20, the amount of calculation becomes unrealistic.

It is suitable for use on small-scale gene sets that have already been preliminarily screened by other methods.

## 环境设置
## Environment settings

```{r}
source("install_dependencies.R") # 如果缺少包，请先运行此脚本安装依赖包 # If you lack a package, please run this script to install the dependencies
library(survival) # 生存分析包，常用于生存数据统计与分析 # Survival analysis package, commonly used for statistical analysis of survival data
library(randomForestSRC) # 随机生存森林，用于变量筛选及建模 # Random survival forest, used for variable selection and modeling
...
## cox回归筛选与OS有关的基因
## Cox regression to screen genes related to OS
...
```{r}
# 取出cox显著的基因做随机生存森林
# Take out the genes that are significant in cox for random survival forest
...
## 排列组合确定最优签名及log-rank检验
## Permutations and combinations determine the optimal signature and log-rank test
...
绘制排列组合p值 | Plot permutations and combinations of p-values
